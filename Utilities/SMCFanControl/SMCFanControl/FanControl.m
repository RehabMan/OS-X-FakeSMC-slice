...
{NSLog(@"upgradeFavorites\n");

	//upgrade favorites
	NSMutableArray *rfavorites = [FavoritesController arrangedObjects];
	int j;
	int i;
	for (i=0;i<[rfavorites count];i++)
	{
		BOOL selected = NO;
		NSMutableArray *fans = [[rfavorites objectAtIndex:i] objectForKey:@"FanData"];
...
-(void)init_statusitem{
	...
	coreFreqState = [[[NSStatusBar systemStatusBar] statusItemWithLength: NSVariableStatusItemLength] retain];
	[coreFreqState setMenu: theMenu];
	[coreFreqState setEnabled: YES];
	[coreFreqState setHighlightMode:YES];
	[coreFreqState setTitle:@"COREFREQ"];
...
-(void) readFanData:(NSTimer*)timer{
	...
	for(i=0; i<[smcWrapper get_fan_num]; i++){
		...
		[[theMenu itemWithTag:(i+1)*10] setTitle:[NSString stringWithFormat:@"Fan %d: %@ RPM",i,[[NSNumber numberWithInt:[smcWrapper get_fan_rpm:i]] stringValue]]];
	}
	...
	switch ([[defaults objectForKey:@"MenuBar"] intValue]) {
		case 0: //Values
			...
			//Core Frequency
			if (core_count) {
				[coreFreqState setLength:NSVariableStatusItemLength];
				someString=[[NSMutableString alloc] init];
				
				for (i=0; i<core_count; i++) {
					[someString appendFormat:@"%d MHz",[smcWrapper get_mainfreq:i]];
					if (i==core_count/2-1)
						[someString appendFormat:@"\n"];
					else 
						[someString appendFormat:@" "];
				...
				[s_status addAttribute:NSStrokeWidthAttributeName value:strWidth range:range];
				[s_status addAttribute:NSForegroundColorAttributeName value:(NSColor*)[NSUnarchiver unarchiveObjectWithData:[defaults objectForKey:@"MenuColor"]]  range:range];
				[coreFreqState setAttributedTitle:s_status];
				[someString release];
			}
			else 
				[coreFreqState setLength:0];
			...
			if (gc) {
				...
				for (i=0; i<gc; i++) {
					...
					sprintf(key, "FGC%d", i);
					t=[smcWrapper getFreq:key];
					if (t>0)
						[someString appendFormat:@"%d MHz ", t];
