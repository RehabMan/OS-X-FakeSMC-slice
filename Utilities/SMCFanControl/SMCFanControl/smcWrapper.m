...
+(int) getFreq:(UInt32Char_t)key{
	SMCVal_t val;
	SMCReadKey2(key, &val, conn);
	return (val.bytes[0]<<8)|(val.bytes[1]&0xff);
}


+(int) get_mainfreq:(int)core_number{
	
	UInt32Char_t  key;
	sprintf(key, "FRC%d", core_number);
	return [smcWrapper getFreq:key];
}
