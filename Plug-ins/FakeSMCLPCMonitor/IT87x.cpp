...
void IT87x::Init() {
	...
	// Heatsink
	if (ITE_HEATSINK_TEMPERATURE_OFFSET>=0)
		FakeSMCAddKeyCallback("Th0H", "sp78", 2, value, &::Update);
	
	// Northbridge
	if (ITE_NORTHBRIDGE_TEMPERATURE_OFFSET>=0)
...
void IT87x::SetTemperatureOffsets(UInt8 hsOffset, UInt8 nbOffset) {
	ITE_HEATSINK_TEMPERATURE_OFFSET=hsOffset;
	ITE_NORTHBRIDGE_TEMPERATURE_OFFSET=nbOffset;
}
...
void IT87x::Update(const char *key, char *data)
{
	...
	{
		char value = ReadByte(ITE_TEMPERATURE_BASE_REG + ITE_HEATSINK_TEMPERATURE_OFFSET, valid);
	...
	{
		char value = ReadByte(ITE_TEMPERATURE_BASE_REG+ITE_NORTHBRIDGE_TEMPERATURE_OFFSET, valid);
