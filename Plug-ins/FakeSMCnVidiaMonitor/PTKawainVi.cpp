...
static void Update(const char* key, char* data) {	
	data[0] = PTKawainViService->update();
	data[1] = 0;
}

int PTKawainVi::update() {
	int nVtemp;
...
PTKawainVi::start( IOService * provider ) {
	...
	IOLog("Warning: this will load on ANY GPU (incl. ATI)\n");
	...
	if (!NVCard) {
		IOLog("PTKawainVi: no graphics adapter found\n");
		return false;
	}
