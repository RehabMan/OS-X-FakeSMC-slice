...
void PTKawainVi::parse_bit_temperature_table(int offset) {
	...
	//IOLog("PTKawainVi: Temperature table version: %#x\n", header->version);
	//IOLog("PTKawainVi: Correction: %d\n", correction);
	//IOLog("PTKawainVi: Offset: %d\n", offset);
	//IOLog("PTKawainVi: Slope: %d\n", (int)slope);
}
...
PTKawainVi::start( IOService * provider ) {
	...
	switch (arch) {
		...
		case 0x96:
			...
			FakeSMCAddKeyCallback(key, "fpe2", 2, value, &::UpdateF);
			UpdateFNum(1);
	}
